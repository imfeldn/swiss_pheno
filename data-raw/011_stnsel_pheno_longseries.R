...
for(pv in phenovals){
  ...
  early_stns <- sapply(vval_stns, function(x){nrow(pheno_sel[pheno_sel$nat_abbr == x & pheno_sel$reference_year < 1988,])}) > 15
  late_stns <- sapply(vval_stns, function(x){nrow(pheno_sel[pheno_sel$nat_abbr == x & pheno_sel$reference_year > 1990,])}) > 15

  longseries <- vval_stns[(early_stns + late_stns) == 2]
  pheno_sel_long <- pheno_sel[pheno_sel$nat_abbr %in% longseries,] ## from these only select long series

  station_list_sel <- station_list[as.character(station_list$Abk.) %in% longseries,1:10]
  station_list_sel <- station_list_sel[longseries,] # reorder station list according to vvals

  # save as compressed RDS
  saveRDS(
    station_list_sel,
    file = paste0("data-raw/02_pheno_net/stations_",pv,"_class",cllps(classes),"_longseries.rds"),
    compress = "xz"
    )
  ...
  pheno_dat <- cbind(pheno_sel_long, lon, lat, E, N, alt)

  ### add qc flags ## most of them are already remove though
  qc_pv <- qc[qc$OLD_SHORT_TX == pv & qc$FinalFlagCountUnique == 1 & qc$station %in% longseries,c("station","year","doy","value","FinalFlagCountUnique")]

  for(ii in 1:nrow(qc_pv)){
    ind <- which(pheno_dat$nat_abbr == qc_pv[ii,"station"] & pheno_dat$reference_year == qc_pv[ii,"year"])
    if(length(ind) > 0){
      print(pheno_dat[ind,])
      pheno_dat[ind,"doy"] <- NA
    }
  }
  ...
  saveRDS(
    pheno_dat,
    file = paste0("data-raw/02_pheno_net/pheno_dat_",pv,"_class",cllps(classes),"_longseries.rds"),
    compress = "xz"
    )
}
