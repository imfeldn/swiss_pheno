...
### select stations based on classes
...
for(pv in phenovals){
  ...
  saveRDS(
    station_list_sel,
    file = paste0("data-raw/02_pheno_net/stations_",pv,"_class",cllps(classes),".rds"),
    compress = "xz"
  )
  ...
  pheno_dat <- cbind(pheno_sel, lon, lat, E, N, alt)
  ...
  saveRDS(
    pheno_dat,
    file = paste0("data-raw/02_pheno_net/pheno_dat_",pv,"_class",cllps(classes),".rds"),
    compress = "xz"
  )
